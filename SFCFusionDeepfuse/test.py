...
def test(test_path, model, img_type='gray', save_path='./',mode='l1',window_width=1):
    ...
    for i in range(int(img_num)):
        ...
        img3_path = test_path+img_name_list[0]+str(i+1)+'.'+suffix
        img4_path = test_path+img_name_list[1]+str(i+1)+'.'+suffix
        save_name = 'fusion'+str(i+1)+'_'+img_type+'_'+mode+'.'+suffix
        if img_type =='gray':
            fusion_phase.get_fusion(img1_path,img2_path,img3_path,img4_path,model,
