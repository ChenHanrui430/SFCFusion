...
def fusion(x1,x2,x3,x4,x5,x6,model,mode='l1', window_width=1,D=None):
    with torch.no_grad():
        ...
        if D is not None:
            ...
            weightMapMin_final = torch.where(torch.isnan(weightMapMin), torch.full_like(weightMapMin, 1), weightMapMin)
            featureSBF = featureSF+weightMapMin_final*(weightMapMax_final-weightMapMin_final)/(weightMapMax_final.add(9))# 防止除0
            feature_fusion = featureSBF+ (w1/W * feature5 +w2/W * feature6)-(w1*feature3+w2*feature4)*1.001
        else:
...
class test_gray(Test):
    ...
    def get_fusion(self,img1_path,img2_path,img3_path,img4_path,img5_path,img6_path,model,
        ...
        img1, img2 = self.load_imgs(img1_path,img2_path,device) # 可见基础层，红外基础层
        img3, img4 = self.load_imgs(img3_path,img4_path,device) #可见显著，
        img5, img6 = self.load_imgs(img5_path,img6_path,device) #可见原图，红外原图
        Dlist =torch.rand(4)
        Dlist=Dlist.cuda()
...
def test(test_path, model, img_type='gray', save_path='./test_result/',mode='l1',window_width=1):
    ...
    for i in range(int(img_num)):
        ...
        img5_path = test_path+img_name_list[5]+str(i+1)+'.'+suffix
        img6_path = test_path+img_name_list[4]+str(i+1)+'.'+suffix
